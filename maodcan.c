...
double dijkstra(Graph *graph, Node *start, Node *end) {
    int i;
    Node *neighbor;
    Node *node;
    Node *current;
    ...
    for(i = 0; i < graph->n_nodes; i++) {
        ...
        previous[i] = -1;
        visited[i] = 0; /* set ith node a0s unvisited */
    }
...
int tsp(Graph *graph, char list [], int len){
    ...
    for(int d=1; d<len-1; d++) {
        Node *save_id1;
        for (int i = 0; i <graph->n_nodes ; ++i) {
            if(graph->nodes[i]->label==list[d]){
                save_id1=graph->nodes[i];
            }
        }
        Node *save_id2;
        for (int i = 0; i <graph->n_nodes ; ++i) {
            if(graph->nodes[i]->label==list[d+1]){
                save_id2=graph->nodes[i];
            }
        }

        w+=dijkstra(graph,save_id1, save_id2);
        if(w>10000){
         return -1;
        }
    }
